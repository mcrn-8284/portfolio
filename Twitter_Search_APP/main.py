...
# idの履歴を取得する関数
def read_id():
    if os.path.exists('main_data/id.txt'):
        with open("main_data/id.txt", mode="r") as f:
            return f.readlines()
    else :
        with open("main_data/id.txt", mode="w") as f:            
            f.write("")
    return []

############################################################

def make_user_dataflame(user_id, max_get_tweets_count=1000):
    ...
    if array_tweets_from_json[0]['user']['id_str'] + '\n' in read_id():
        ...
        new_person = True
        with open("main_data/id.txt", mode = "a") as f:
            f.write(array_tweets_from_json[0]['user']['id_str'] + '\n')

    # ユーザーデータフレームの読み込み
    if not os.path.exists('main_data/user.csv'):
        df = pd.DataFrame(
            columns=["id_str", "name", "introduction", "created_at"]
        )
        df.to_csv('main_data/user.csv', index = False)
    df = pd.read_csv('main_data/user.csv')
...
def your_similar_user(user_id_lst):
    ...
    if flag:
        user_vec_lst = joblib.load('main_data/user_vec_lst.sav')
    else :
        autosave_user_data()
        loaded_clf = joblib.load('main_data/model.sav')
    ...
    user_df = pd.read_csv('main_data/user.csv')
    similar_user = user_df[
        user_df['id_str'] == int(
            user_id_lst[cos_dis.index(max(cos_dis)) + 1]
        )
    ]['name'].values[0]
    return similar_user
...
if __name__ == '__main__':
    import sys
    args = sys.argv
    # python main.py [register_or_calc] [user_id]
    # argv -> 0:main.py / 1:[register_or_calc] / 2:[user_id]
    if len(args) < 2:
        print('引数が足りません。', file=sys.stderr)
    if not args[1] in ['register', 'calc']:
        print('引数１には"register"または"calc"を入力してください。', file=sys.stderr)
    if args[1] == 'register' and len(args) < 3:
        print('引数が足りません。', file=sys.stderr)
    if args[1] == 'register' and not args[2].isdigit():
        print('UserIDはすべて数字でなければなりません。', file=sys.stderr)

    is_register = args[1] == 'register'
    if is_register:
        make_user_dataflame(args[2])
    else:
        ids = read_id()
        print(ids)
        user_id_list = list(map(lambda x: x.split('\n')[0], ids))
        similar_user = your_similar_user(user_id_list)
        print('最も似ているユーザー：' + similar_user)
